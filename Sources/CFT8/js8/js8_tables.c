// LDPC(174,87) code tables used by JS8 (the original 2017/2018 FT8 code).
//
// Copied verbatim from ft8_lib by Kārlis Goba, commit 0edb70c94b ("Working
// decode (v1)", 2018-11-20), MIT License — see LICENSE.ft8_lib in this
// directory's parent. ft8_lib in turn transcribed the numbers from WSJT-X's
// ldpc_174_87_params.f90 / bpdecode174.f90. These are data tables that
// define the code; no GPL source is included here.
#include "js8_tables.h"

/// Parity generator matrix for the (174,87) LDPC code, bit-packed MSB first.
/// Row i gives the 87 message bits that XOR to parity bit i.
...
/// Column order (permutation) in which the bits of the codeword are stored.
/// itmp = [87 parity bits, 87 message bits]; codeword[colorder[i]] = itmp[i].
...
/// Parity check matrix: 87 rows, each lists the (1-origin) codeword bits that
/// must XOR to zero.
...
/// For each of the 174 codeword bits, the three (1-origin) parity checks
/// that include it.
...
/// Number of valid entries in each row of kJS8_LDPC_Nm.
